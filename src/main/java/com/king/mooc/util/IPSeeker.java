...
public class IPSeeker {
    ...
    private static final String IP_FILE = "/king/springboot/util/qqwry.dat";
...
//            System.out.println(Objects.requireNonNull(IPSeeker.class.getResource("/QQWry.dat")).toString());
        ...
        if (ipFile != null) {
            try {
                ...
                logger.info("IP地址信息文件格式有错误，IP显示功能将无法使用");
                ipFile = null;
            }
        }
    }
    ...
    public String getAddress(byte[] ip) {
        String country = getCountry(ip).equals(" CZ88.NET") ? "" : getCountry(ip);
        String area = getArea(ip).equals(" CZ88.NET") ? "" : getArea(ip);
        String address = country + " " + area;
        return address.trim();
    ...
    public List<String> getAddress(String... ips) {
        List<String> list = new ArrayList<>();
        for (String ip : ips) {

            list.add((getCountry(ip).equals(" CZ88.NET") ? "" : getCountry(ip)) + " " + (getArea(ip).equals(" CZ88.NET") ? "" : getArea(ip)));
        }
...
//        list.add("8.8.8.8");
//       logger.info(ips.getAddress(list));
//
//
//        String ip = "218.77.74.132";
//       logger.info(ips.getAddress(ip));
//        byte[] ipb= {8,8,8,8};
//       logger.info(ips.getAddress(ipb));
//
//       logger.info( IPSeeker.getInstance().getAddress(ip));
    }

    private List<String> getAddress(List<String> list) {
        List<String> lists = new ArrayList<>();
        for (String ip : list) {
            lists.add((getCountry(ip).equals(" CZ88.NET") ? "" : getCountry(ip)) + " " + (getArea(ip).equals(" CZ88.NET") ? "" : getArea(ip)));
        }
